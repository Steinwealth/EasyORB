# Updated Priority Rank Formula for ORB Strategy

**Date**: January 6, 2026  
**Status**: ✅ **READY FOR IMPLEMENTATION**  
**Current Version**: v2.1 (Rev 00106, Nov 6, 2025)  
**Proposed Version**: v2.2 (Jan 6, 2026)

---

## Executive Summary

The current Priority Rank Formula (v2.1) does **NOT** include ORB Volatility Scoring, which is a proven predictor used in the 0DTE Strategy. This document provides the **complete updated formula** with ORB Volatility Scoring included.

---

## Current Formula (v2.1) - Rev 00106
...
The current formula uses `orb_range_score` (3% weight) which is based on raw ORB Range %: `(orb_high - orb_low) / orb_low`. This is **NOT** the same as ORB Volatility scoring.

---

## Updated Formula (v2.2) - Recommended

### **Option 1: Replace ORB Range with ORB Volatility Scoring** ⭐ RECOMMENDED
...
Priority Score = 
  ...
  Confidence (13%) +
  RSI (10%) +
  ORB Volatility Scoring (3%)  ← REPLACED ORB Range
```

**Rationale**:
- Maintains same total weight (100%)
- Replaces weak predictor (ORB Range %) with stronger predictor (ORB Volatility)
- Consistent with 0DTE Strategy approach
- Minimal disruption to existing formula
...
- **Primary**: `entry_bar_volatility` - Volatility at entry bar (calculated from ORB high/low)
